...
class EnemyKrazownik:
    ...
    def update(self, window, ENEMY,LASERY_ENEMY,RAKIETY_ENEMY, rodzajMisji):
        ...
        if pygame.time.get_ticks() - self.czasStop > 750 and self.stop:
            ...
            self.rakietaD.play()
            if rodzajMisji in [1, 2]:
                RAKIETY_ENEMY.append(Rakieta(self.x+self.szerokosc//2,self.y+self.wysokosc//2+10,0,3,180,100,rodzajMisji))
            if rodzajMisji in [3, 4]:
                RAKIETY_ENEMY.append(Rakieta(self.x + 20, self.y + self.wysokosc // 2 , -3, 0, 90,100, rodzajMisji))
